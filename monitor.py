...
get_active_client() ниже отдаёт текущий подключённый клиент этого цикла -
через него другие части бота (main.py) переиспользуют то же самое Telethon-
подключение вместо того, чтобы открыть второй TelegramClient на тот же файл
сессии (что ловит `sqlite3.OperationalError: database is locked`, см.
userbot_manager.py).

Группы хранятся не как простые ссылки, а как структура {chat_ref,
start_message_id} - см. groups.py: администратор может указать ссылку на
группу, ссылку на конкретное сообщение (в т.ч. в приватной группе без
username), @username или просто числовой id.
"""
import asyncio
import logging
...
from groups import load_groups, resolve_entity, describe
...
async def _listen_until_disconnected(client, groups: list[dict]):
    entities = []
    for group in groups:
        try:
            entity = await resolve_entity(client, group["chat_ref"])
            entities.append(entity)
        except Exception as e:
            log.warning("Не удалось получить группу %s: %s", describe(group), e)
...
async def _run_forever():
    ...
    """
    global _should_run, _current_client
    while _should_run:
        ...
        # публикуем клиент СРАЗУ, ещё до старта прослушивания - чтобы
        # другие части бота (проверка группы при добавлении, статус) могли
        # переиспользовать это же подключение вместо второго TelegramClient
        # на тот же файл сессии
...
def get_active_client():
    """
    Возвращает текущий подключённый Telethon-клиент активного цикла
    мониторинга, либо None, если мониторинг сейчас не подключён (ждёт
    входа/групп, или между переподключениями). Используется
    userbot_manager.get_ready_client("monitor"), чтобы переиспользовать
    это же подключение вместо открытия второго на тот же файл сессии.
    """
