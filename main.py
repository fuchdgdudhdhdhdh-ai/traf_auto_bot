...
from groups import load_groups, parse_group_input, same_chat, resolve_entity, describe
...
async def status_cb(call: CallbackQuery):
    groups = load_groups(settings.GROUPS_FILE)
    subs = load_json(settings.SUBSCRIBERS_FILE, [])
    msg = load_json(settings.BROADCAST_MESSAGE_FILE, None)

    # ВАЖНО: monitor_client тут может оказаться тем же живым клиентом, что
    # держит 24/7 мониторинг (см. userbot_manager.py) - поэтому отпускаем
    # его через release_client(), а не client.disconnect() напрямую, иначе
    # оборвём мониторинг только чтобы проверить статус.
    monitor_client = await get_ready_client("monitor")
    monitor_authorized = monitor_client is not None
    await release_client("monitor", monitor_client)

    broadcast_client = await get_ready_client("broadcast")
    broadcast_authorized = broadcast_client is not None
    await release_client("broadcast", broadcast_client)
    ...
    if groups:
        shown = [describe(g) for g in groups[:10]]
...
async def login_monitor_cb(call: CallbackQuery, state: FSMContext):
    ...
    if client:
        await release_client("monitor", client)
...
async def addgroup_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "Отправьте группу(ы) для мониторинга, можно несколько строк за раз. "
        "Поддерживаются форматы:\n\n"
        "• ссылка на сообщение (предпочтительно) - откройте любое сообщение "
        "в группе → «Копировать ссылку», например https://t.me/mygroup/123 "
        "или https://t.me/c/1234567890/123 (для приватных групп без "
        "username - тоже подходит);\n"
        "• обычная ссылка на группу: https://t.me/mygroup;\n"
        "• @username группы;\n"
        "• числовой id группы: -1001234567890."
    )
...
async def addgroup_save(message: Message, state: FSMContext):
    ...
    added, duplicates, unresolved = [], [], []
    for line in lines:
        parsed = parse_group_input(line)
        if any(same_chat(parsed, g) for g in groups):
            duplicates.append(line)
            continue

        # если аккаунт мониторинга уже авторизован - сразу проверяем, что
        # группа реально доступна, чтобы дать обратную связь сейчас, а не
        # молча ждать следующей попытки подключения мониторинга
        if monitor_client:
            try:
                await resolve_entity(monitor_client, parsed["chat_ref"])
            except Exception as e:
                unresolved.append(f"{line} ({e})")
                continue

        groups.append(parsed)
        added.append(line)

    # monitor_client тут обычно и есть тот самый живой клиент 24/7
    # мониторинга (см. userbot_manager.py) - release_client() его не
    # тронет, отключит только если это был отдельный, специально открытый
    # для этой проверки клиент (мониторинг ещё не был подключён)
    await release_client("monitor", monitor_client)
    ...
    report = f"Добавлено групп: {len(added)}. Всего в списке: {len(groups)}."
    if duplicates:
        report += f"\nУже были в списке (пропущено): {len(duplicates)}."
    if unresolved:
        report += "\n⚠️ Не удалось получить доступ (проверьте, что аккаунт мониторинга состоит в группе):\n" + "\n".join(unresolved)
    if added:
        report += "\nМониторинг (24/7) подхватит их в течение нескольких секунд."
...
async def do_broadcast_user(message: Message, client):
    ...
    if not msg or not msg.get("text") or not subs:
        ...
        await release_client("broadcast", client)
        return
